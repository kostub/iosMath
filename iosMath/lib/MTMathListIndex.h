 ...
 */
typedef NS_ENUM(unsigned int, MTSubIndexType) {
    ...
    /// Index into MTInner (only valid for MTInner)
...
@property (nonatomic, readonly) MTSubIndexType subIndexType;
...
- (BOOL) isAffectedByChangesIn:(nonnull MTMathListIndex *)other;
...
- (nonnull MTMathListIndex *) addingFinalIndex:(NSInteger) index;
...
- (nonnull MTMathListIndex*) levelUpWithSubIndex:(nullable MTMathListIndex*) subIndex type:(MTSubIndexType) type;
 ...
 */
+ (nonnull instancetype) indexAtLocation:(NSUInteger) location withSubIndex:(nullable MTMathListIndex*) subIndex type:(MTSubIndexType) type;
