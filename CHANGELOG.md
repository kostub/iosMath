...
### v2.6.0 (2026-07-28)
* Add amsmath's **argument-free macros**. `\implies`, `\iff`, and `\impliedby` now carry amsmath's `\;` on both sides instead of rendering as the bare arrow. New: the named spacings `\thinspace`, `\medspace`, `\thickspace`, `\negthinspace`, `\negmedspace`, `\negthickspace`; the multiple integral `\idotsint`; and the limit forms `\varliminf`, `\varlimsup`, `\varinjlim`, `\varprojlim` (#273). The `\var…lim` family draws the right symbol but is an ordinary atom rather than an operator, because iosMath has no `\mathop`: a script lands to its right rather than centred underneath, and it loses the 3mu an operator gets against what follows it.
